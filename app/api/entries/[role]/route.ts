...
export async function PUT(req: Request, ctx: { params: { role: string } }) {
  ...
  const children1_gratitude = String(body.children1_gratitude ?? "").trim();
  const children2_gratitude = String(body.children2_gratitude ?? "").trim();

  if (!general_1 || !general_2 || !partner_specific || !children1_gratitude || !children2_gratitude) {
    return bad("#1–#5 dankbar müssen ausgefüllt werden.", 400);
  }

  upsertEntryForToday(role, {
    general_1,
    general_2,
    partner_specific,
    children1_gratitude,
